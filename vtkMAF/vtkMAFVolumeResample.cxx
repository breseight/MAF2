...
/*
void vtkMAFVolumeResample::myDBG(const char *msg,int index)
{
  cerr<<"myDBG: ";
  if (msg)
    cerr<<msg<<" ";
...
//----------------------------------------------------------------------------
void vtkMAFVolumeResample::SetVolumeAxisX(double axis[3])
{
...
//----------------------------------------------------------------------------
void vtkMAFVolumeResample::SetVolumeAxisY(double axis[3]) {
  if (vtkMath::Norm(axis) < 1.e-5f)
    return;
...
//----------------------------------------------------------------------------
void vtkMAFVolumeResample::CalculateTextureCoordinates(const double point[3], const int size[2], const double spacing[2], double ts[2]) {
