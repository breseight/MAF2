...
public:
  ...
  vtkMAFOrientationMarkerWidget *OrientationMarkerWidget;
};
...
vtkMAFOrientationMarkerWidget::vtkMAFOrientationMarkerWidget()
{
  ...
  this->State = vtkMAFOrientationMarkerWidget::Outside;
  this->Interactive = 1;
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::SetEnabled(int enabling)
{
  if (!this->Interactor)
    {
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::ExecuteCameraUpdateEvent(vtkObject *vtkNotUsed(o),
...
//-------------------------------------------------------------------------
int vtkMAFOrientationMarkerWidget::ComputeStateBasedOnPosition(int X, int Y,
                                                    int *pos1, int *pos2)
{
  if ( X < (pos1[0]-this->Tolerance) || (pos2[0]+this->Tolerance) < X ||
       Y < (pos1[1]-this->Tolerance) || (pos2[1]+this->Tolerance) < Y )
    {
    return vtkMAFOrientationMarkerWidget::Outside;
    }

  // if we are not outside and the left mouse button wasn't clicked,
  // then we are inside, otherwise we are moving

  int result = this->Moving ? vtkMAFOrientationMarkerWidget::Translating :
      vtkMAFOrientationMarkerWidget::Inside;
  ...
  if (e1)
    ...
    if (e2)
      {
      result = vtkMAFOrientationMarkerWidget::AdjustingP1; // lower left
      }
    if (e4)
      {
      result = vtkMAFOrientationMarkerWidget::AdjustingP4; // upper left
      }
    }
  if (e3)
    {
    if (e2)
      {
      result = vtkMAFOrientationMarkerWidget::AdjustingP2; // lower right
      }
    if (e4)
      {
      result = vtkMAFOrientationMarkerWidget::AdjustingP3;  // upper right
      }
    }
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::SetCursor(int state)
{
  switch (state)
    {
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::ProcessEvents(vtkObject* vtkNotUsed(object),
                                    unsigned long event,
                                    void *clientdata,
                                    void* vtkNotUsed(calldata))
{
  vtkMAFOrientationMarkerWidget *self =
    reinterpret_cast<vtkMAFOrientationMarkerWidget*>( clientdata );
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::OnLeftButtonDown()
{
  ...
  if (this->State == vtkMAFOrientationMarkerWidget::Outside)
    {
    this->Moving = 0;
    return;
    }
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::OnLeftButtonUp()
{
  if (this->State == vtkMAFOrientationMarkerWidget::Outside)
    {
    return;
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::SquareRenderer()
{
  ...
  if (dx != dy)
    ...
    switch (this->State)
      {
      case vtkMAFOrientationMarkerWidget::AdjustingP1:
        vp[2] = vp[0] + delta;
        vp[3] = vp[1] + delta;
        break;
      case vtkMAFOrientationMarkerWidget::AdjustingP2:
        vp[0] = vp[2] - delta;
        vp[3] = vp[1] + delta;
        break;
      case vtkMAFOrientationMarkerWidget::AdjustingP3:
        vp[0] = vp[2] - delta;
        vp[1] = vp[3] - delta;
        break;
      case vtkMAFOrientationMarkerWidget::AdjustingP4:
        vp[2] = vp[0] + delta;
        vp[1] = vp[3] - delta;
        break;
      case vtkMAFOrientationMarkerWidget::Translating:
        delta = (dx + dy)*0.5;
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::UpdateOutline()
{
  double vp[4];
  this->Renderer->GetViewport( vp );
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::SetInteractive(int interact)
{
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::OnMouseMove()
{
  ...
  if (this->State == vtkMAFOrientationMarkerWidget::Outside || !this->Moving)
    {
    this->Interactor->Render();
    return;
    }
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::MoveWidget(int X, int Y)
{
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::ResizeTopLeft(int X, int Y)
{
  int dx = X - this->StartPosition[0];
  int dy = Y - this->StartPosition[1];
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::ResizeTopRight(int X, int Y)
{
  int dx = X - this->StartPosition[0];
  int dy = Y - this->StartPosition[1];
  int delta = (abs(dx) + abs(dy))/2;
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::ResizeBottomRight(int X, int Y)
{
  int dx = X - this->StartPosition[0];
  int dy = Y - this->StartPosition[1];
  int delta = (abs(dx) + abs(dy))/2;
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::ResizeBottomLeft(int X, int Y)
{
  int dx = X - this->StartPosition[0];
  int dy = Y - this->StartPosition[1];
  int delta = (abs(dx) + abs(dy))/2;
...
//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::SetOutlineColor(double r, double g, double b)
{
...
//-------------------------------------------------------------------------
double* vtkMAFOrientationMarkerWidget::GetOutlineColor()
{
  return this->OutlineActor->GetProperty()->GetColor();
}

//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::SetViewport(double minX, double minY,
...
//-------------------------------------------------------------------------
double* vtkMAFOrientationMarkerWidget::GetViewport()
{
  return this->Renderer->GetViewport();
}

//-------------------------------------------------------------------------
void vtkMAFOrientationMarkerWidget::PrintSelf(ostream& os, vtkIndent indent)
{
