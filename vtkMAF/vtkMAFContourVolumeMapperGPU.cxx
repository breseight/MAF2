...
//{
  ...
  //baoquan 's function
#pragma region Baoquans GPU Misc
#ifdef _WIN32
  //------------------------------------------------------------------------
...
#define CHECK_GL \
  ...
  void
    dumpSourceWithLineNumbers( const std::string& src , fstream & ofs)
  {
    int line = 1;
    ...
    compileShader( const std::string& source, GLuint type )
  {
    ...
    fstream ofs("obj_transform.txt", ios::out );    
    ...
    {
      ofs << "*** Source ***\n";
      dumpSourceWithLineNumbers( source,  ofs);
      ofs << "*** Compile error ***\n";
      GLint logsize;
      ...
      if(logsize > 0 ) {
        ...
        ofs << std::string( infolog.begin(), infolog.end() ) << "\n";
      } else {
        ofs << "empty log.\n";
      }
      ofs.close();
  ...
  bool assertExtension( const std::string& extension )
  {
    std::cerr << extension << ": ";
...
#define __func__ __FUNCTION__
  ...
  void vtkMAFContourVolumeMapperGPU::DeleteTexture()
  {
    if(m_tex_function>0)
    {	
      ...
      glDeleteTextures( 1, &m_hp_tex );
      m_hp_tex=0;


      glDeleteFramebuffersEXT(m_hp_size_log2+1, &m_hp_framebuffers[0]);
      m_hp_framebuffers.clear();

      glDeleteObjectARB(m_onscreen_v);
      glDeleteObjectARB(m_onscreen_f);
      glDeleteObjectARB(m_onscreen_p);

      glDeleteObjectARB(m_gpgpu_passthru_v);
      glDeleteObjectARB(m_baselevel_f);
      glDeleteObjectARB(m_baselevel_p);

      glDeleteObjectARB(m_reduce_f);
      glDeleteObjectARB(m_reduce_p);

      glDeleteBuffers( 1, &m_enumerate_vbo );
      m_enumerate_vbo=0;
    }
  }
  ...
  template <typename DataType> void vtkMAFContourVolumeMapperGPU::PaddingMem( DataType *dataPointer)
  {

    int sizeofData=sizeof(DataType);
    GLuint srcWidth=DataDimensions[0];
    GLuint srcPage=srcWidth *DataDimensions[1];

    GLuint pageAmount=m_function_tsize * m_function_tsize;
    GLuint allAmount=pageAmount * m_function_slices;
    pPading = new DataType [allAmount];
    memset(pPading, 0 ,  allAmount*sizeofData);

    DataType *pFirst=(DataType *)pPading;
    DataType *pSrc=dataPointer;

    DataType *pDst;

    for(int k=0; k<DataDimensions[2]; k++) //page
    {
      for(int j=0; j<DataDimensions[1]; j++) //row
      {
        pDst=pFirst+ k*pageAmount + j*m_function_tsize;

        memcpy(pDst, pSrc, srcWidth* sizeofData);
        pSrc+= srcWidth;
      }
    }
  }
  ...
  //------------------------------------------------------------------------------
  /*virtual*/ void vtkMAFContourVolumeMapperGPU::SetGPUEnabled(int newMode)
    //------------------------------------------------------------------------------
  {
    if (this->GPUEnabled != newMode)
    {
      this->GPUEnabled = newMode;
      this->Modified();
      if(!newMode)//to CPU
      {
        bBuildHp=0;
  ...
  int  vtkMAFContourVolumeMapperGPU::CreateTexturesForIsosurface(const void *dataPointer)
  {	
    if(m_bGPU_isosurfaceSupported==0 || m_tex_function)
      return 0;

    m_function_tsize_log2 = (unsigned int)ceilf(log2(std::max(this->DataDimensions[0] , this->DataDimensions[1])));
    m_function_tsize = 1<<m_function_tsize_log2;

    //m_function_slices = 1<<((unsigned int)ceilf(log2(ceilf( this->DataDimensions[2]   ))));
    m_function_slices = DataDimensions[2]  ;


    int cols_log2 = (int)ceilf(log2(sqrt((double)m_function_slices)));
    m_hp_size_log2 = cols_log2 + m_function_tsize_log2-1;
    m_slice_level=	(cols_log2-1);
    m_hp_size = 1<<m_hp_size_log2;
    ...
    case VTK_CHAR:
      ...
      type = GL_BYTE;
      stride = sizeof(char);
      m_isovalue_scale = 1.0f/VTK_CHAR_MAX;
      PaddingMem((char*) dataPointer);
    ...
    case VTK_UNSIGNED_CHAR:
      ...
      PaddingMem((unsigned char*) dataPointer);
      break;
    ...
    case VTK_SHORT:
      ...
      type = GL_SHORT;
      stride = sizeof( short);
      m_isovalue_scale = 1.0f/VTK_SHORT_MAX;
      PaddingMem(( short*) dataPointer);
    ...
    case VTK_UNSIGNED_SHORT:
      ...
      PaddingMem((unsigned short*) dataPointer);
      break;
    ...
    case VTK_FLOAT:
      ...
      PaddingMem((float*) dataPointer);
      break;

    case VTK_DOUBLE:
      // break;
    default:
      vtkWarningMacro(<< "Only 8/16 bit integers and 32 bit floats are supported.");
      m_bGPU_isosurfaceSupported=0;
      return 0;
    ...
    //if(sliceSize*slices * stride > mountVMEM *0.90)
    if( (m_function_slices*m_function_tsize*m_function_tsize * stride + (1.4)*m_hp_size*m_hp_size*4*4 ) > (mountVMEM-50000000 ) )//50,000,000 meens 50M for other texture useness
    {
      vtkErrorMacro(<< "Not enough video memory.");
      m_bGPU_isosurfaceSupported=0;
      delete [] pPading;
      return 0;
    }
    ...
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA32F_ARB, m_hp_size, m_hp_size, 0, GL_RGBA, GL_FLOAT, 0 );
    ...
    for(int m=0; m<=m_hp_size_log2; m++) 
    {
      ...
      int bb=checkFramebufferStatus( __func__ );
      if(!bb)		
      {
        m_bGPU_isosurfaceSupported=0;
        vtkErrorMacro(<< "fbo failed.");
        return 0;
      }
    ...
    defines << "#define FUNC_TSIZE " << m_function_tsize << "\n";
    defines << "#define FUNC_SLICES " << m_function_slices << "\n";
    ...
    m_gpgpu_passthru_v = compileShader( snarfFile( str + "iso-surface/vertexpassthrough.glshader" ), GL_VERTEX_SHADER );
    if(m_gpgpu_passthru_v==0)
    {
      vtkWarningMacro(<< "Shader not found.");
      m_bGPU_isosurfaceSupported=0;
      return 0;
    }
    ...
    m_baselevel_f = compileShader( m_defines + snarfFile( str + "iso-surface/hpbaselevel.cg" ), GL_FRAGMENT_SHADER );
    if(m_baselevel_f==0)
    {
      vtkWarningMacro(<<"Shader not found.");
      m_bGPU_isosurfaceSupported=0;
    ...
    int bb=linkProgram( m_baselevel_p );

    glUseProgram( m_baselevel_p );
    m_uniform_baselevel_threshold = getUniformLocation( m_baselevel_p, "threshold" );
    glUniform1i( getUniformLocation( m_baselevel_p, "function_tex" ), 0 );
    glUseProgram( 0 );



    // ---  reduction shader	
    m_reduce_f = compileShader( snarfFile( str + "iso-surface/hpreduction.glshader" ),
      GL_FRAGMENT_SHADER );
    if(m_reduce_f==0)
    {
      vtkWarningMacro(<<"Shader not found.");
    ...
    glUniform1i( getUniformLocation( m_reduce_p, "hp_tex" ), 0 );
    ...
    //DisableRenderingContext();

    // --- onscreen shader
    m_onscreen_v = compileShader( m_defines + snarfFile( str + "iso-surface/vertexexpand_lbq.cg" ), GL_VERTEX_SHADER );
    if(m_onscreen_v==0)
    {
      vtkWarningMacro(<<"Shader not found.");
      m_bGPU_isosurfaceSupported=0;
    ...
    m_onscreen_f = compileShader( m_defines + snarfFile( str + "iso-surface/IsoPhong.cg" ), GL_FRAGMENT_SHADER );
    if(m_onscreen_f==0)
    {
      vtkWarningMacro(<<"Shader not found.");
      m_bGPU_isosurfaceSupported=0;
    ...
    m_onscreen_p = glCreateProgram();
    glAttachShader( m_onscreen_p, m_onscreen_v );
    glAttachShader( m_onscreen_p, m_onscreen_f );
    linkProgram( m_onscreen_p );
    glUseProgram( m_onscreen_p );

    glUniform1i( getUniformLocation( m_onscreen_p, "hp_tex" ), 0 );
    glUniform1i( getUniformLocation( m_onscreen_p, "function_tex" ), 1 );

    m_onscreen_ms_eye = getUniformLocation( m_onscreen_p, "ms_eye" );
    m_onscreen_ms_spacing = getUniformLocation( m_onscreen_p, "ms_spacing" );
    m_onscreen_ms_center = getUniformLocation( m_onscreen_p, "ms_center" );
    ...
    m_uniform_onscreen_func_delta = getUniformLocation( m_onscreen_p, "func_delta" );


    float x_dist=1.0/m_function_tsize;
    float z_dist=1.0/m_function_slices;
    float cell_half_length=0.5*sqrt(x_dist*x_dist + x_dist*x_dist + z_dist*z_dist); 

    glUniform4f( m_uniform_onscreen_func_delta, x_dist,x_dist,z_dist, cell_half_length);
    glUseProgram( 0 );
    ...
    glBindBufferARB( GL_ARRAY_BUFFER, m_enumerate_vbo );
    glBufferData( GL_ARRAY_BUFFER, sizeof(GLfloat)*dummy.size(), &dummy[0], GL_STATIC_DRAW );

    int nParam_ArrayObjectSize = 0;
    glGetBufferParameterivARB( GL_ARRAY_BUFFER, GL_BUFFER_SIZE_ARB, &nParam_ArrayObjectSize );
    if( nParam_ArrayObjectSize <= 0 )
    {
      //vtkWarningMacro(<<"Shader not found.");
      vtkErrorMacro(<<"glBufferDataARB failed to allocate any memory!");
  ...
  int vtkMAFContourVolumeMapperGPU::BuildHistopyramidSliceSorting()
  {
    ...
    // --- base level
    glUseProgram( m_baselevel_p );
    glUniform1f( m_uniform_baselevel_threshold, m_isovalue_scale * ContourValue );
    glBindFramebufferEXT( GL_FRAMEBUFFER_EXT, m_hp_framebuffers[0] );
    glViewport( 0, 0, m_hp_size, m_hp_size );
    ...
    // --- all the reductions
    glUseProgram( m_reduce_p );
    for(int m=1; m<=m_hp_size_log2; m++) 
    {
      glBindTexture( GL_TEXTURE_2D, m_hp_tex );
      ...
      glUniform2f( m_reduce_uniform_delta, -0.5/(1<<(m_hp_size_log2+1-m)), 0.5/(1<<(m_hp_size_log2+1-m)) );
      glBindFramebufferEXT( GL_FRAMEBUFFER_EXT, m_hp_framebuffers[m] );
      glViewport( 0, 0, 1<<(m_hp_size_log2-m), 1<<(m_hp_size_log2-m) );
      glBegin( GL_QUADS );
      glVertex2f( -1.0f, -1.0f );
      glVertex2f(  1.0f, -1.0f );
      glVertex2f(  1.0f,  1.0f );
    ...
    GLfloat mem[4];
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, m_hp_size_log2 );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, m_hp_size_log2 );
    glGetTexImage( GL_TEXTURE_2D, m_hp_size_log2, GL_RGBA, GL_FLOAT, &mem[0] );


    N = (int)(mem[0]+mem[1]+mem[2]+mem[3]);
...
#endif // _WIN32
#pragma endregion //Baoquan's GPU Misc
...
#ifdef _WIN32
    mountVMEM=GetVideoMemorySizeBytes();
#endif // _WIN32
  ...
  void vtkMAFContourVolumeMapperGPU::ReleaseData()
    //------------------------------------------------------------------------------
  {
  ...
  void vtkMAFContourVolumeMapperGPU::ClearCachesAndStats()
    //------------------------------------------------------------------------------
  {
    for (int lod = 0 ;  lod < NumberOfLods ;  lod++){
      // delete triangle caches
  ...
  void vtkMAFContourVolumeMapperGPU::SetInput(vtkDataSet *input)
    //------------------------------------------------------------------------------
  {
    this->ReleaseData();
    double b[2];
    ...
    else
    {
      ...
      spacing[0]=DataSpacing[0];
      spacing[1]=DataSpacing[1];
      spacing[2]=DataSpacing[2];
    }
  ...
  float vtkMAFContourVolumeMapperGPU::EstimateRelevantVolume(const double contourValue)
    //------------------------------------------------------------------------------
  {
    switch (this->GetDataType()) {
    case VTK_CHAR:
  ...
  template <typename DataType> float vtkMAFContourVolumeMapperGPU::EstimateRelevantVolumeTemplate(const DataType ContourValue)
    //------------------------------------------------------------------------------
  {
...
#pragma region //Baoquan's GPU renderer
  ...
  void drawQuads(void)
  {
    ...
    glVertex3f(0.0, extents[1], 0.0);
    glVertex3f(extents[0], extents[1], 0.0);
    glVertex3f(extents[0], 0.0, 0.0);
    ...
    glVertex3f(0.0, 0.0, 0.0);
    glVertex3f(extents[0], 0.0, 0.0);
    glVertex3f(extents[0], 0.0, extents[2]);
    glVertex3f(0.0, 0.0, extents[2]);
    ...
    glVertex3f(0.0, 0.0, 0.0);
    glVertex3f(0.0, 0.0, extents[2]);
    glVertex3f(0.0, extents[1], extents[2]);
    glVertex3f(0.0, extents[1], 0.0);
    glEnd();
  ...
  void vtkMAFContourVolumeMapperGPU::InitializeIsosurfaceRender(bool setup, vtkRenderer *renderer, vtkVolume *volume)
    //------------------------------------------------------------------------------
  {
    ...
    if (setup) 
    {
      ...
      ms_center[0]=extents[0]*0.5;
      ms_center[1]=extents[1]*0.5;
      ms_center[2]=extents[2]*0.5;
  ...
  int  vtkMAFContourVolumeMapperGPU::InitOpenGL()
  {
#ifndef _WIN32
    m_bGPU_isosurfaceSupported=0;
#else
    if(!hOldRC)
    {
      ...
      glDisable(GL_SCISSOR_TEST);//very important
      glDisable(GL_BLEND);
      ...
      if (
        !assertExtension( "GL_EXT_framebuffer_object" ) ||
        !assertExtension( "GL_ARB_texture_float" ) ||
        !assertExtension( "GL_ARB_fragment_shader" ) ||
        !assertExtension( "GL_ARB_vertex_shader" ) ||
        !assertExtension( "GL_EXT_texture3D" ) ||
        !assertExtension( "GL_EXT_gpu_shader4" ) ||
        !assertExtension( "GL_ARB_fragment_program" ) ||
        !assertExtension( "GL_ARB_vertex_program" ) ||
        !assertExtension( "GL_ARB_shader_objects" ) ||
        !assertExtension("GL_ARB_vertex_buffer_object")
        )
      {
        m_bGPU_isosurfaceSupported=0;
        wglMakeCurrent( hOldDC, hOldRC );

        vtkWarningMacro(<< "too old graphics card or driver!");

        return 0;
      }
      DeleteTexture();
      const void *dataPointer = this->GetInput()->GetPointData()->GetScalars()->GetVoidPointer(0);
      int bb=CreateTexturesForIsosurface(dataPointer);	
      if(!bb)
      {
        vtkWarningMacro(<<"create texture failed.");
  ...
  template<typename DataType> void vtkMAFContourVolumeMapperGPU::RenderIsoSurfaceOnGPU(vtkRenderer *renderer, vtkVolume *volume, const DataType *dataPointer)
    //------------------------------------------------------------------------------
  {
    ...
    this->ViewportDimensions[0] = m_viewport[2];
    this->ViewportDimensions[1] = m_viewport[3];
    ...
    InitializeIsosurfaceRender(true, renderer, volume);
    ...
    glUseProgram( m_onscreen_p );  
    glUniform3f( m_uniform_onscreen_threshold, m_isovalue_scale*ContourValue,  
      ViewportDimensions[0],  ViewportDimensions[1] );

    glUniform3fv( m_onscreen_ms_eye, 1, campos_f);
    glUniform3fv( m_onscreen_ms_spacing, 1, spacing);
    glUniform3fv( m_onscreen_ms_center, 1, ms_center);
    ...
    InitializeIsosurfaceRender(false);
  ...
  //              RenderMCubes()
  void vtkMAFContourVolumeMapperGPU::Render(vtkRenderer *renderer, vtkVolume *volume)
    //------------------------------------------------------------------------------
  {
    if (this->GetInput() == NULL || this->GetInput()->GetPointData() == NULL || this->GetInput()->GetPointData()->GetScalars() == NULL) {
    ...
    bool bUseGPU = GPUEnabled != 0 && m_bGPU_isosurfaceSupported != 0;
    if (!bUseGPU)
    {
    ...
    case VTK_CHAR:
      if(bUseGPU)
      {
        this->RenderIsoSurfaceOnGPU(renderer, volume, (const char*)dataPointer);
      }
      else
      {
    ...
    case VTK_UNSIGNED_CHAR:
      ...
      if(bUseGPU)
      {
        this->RenderIsoSurfaceOnGPU(renderer, volume, (const unsigned char*)dataPointer);
      }
      else
      {
    ...
    case VTK_SHORT:
      ...
      if(bUseGPU)
      {
        this->RenderIsoSurfaceOnGPU(renderer, volume, (const short*)dataPointer);
      }
      else
      {
        if (PrepareAccelerationDataTemplate((const short*)dataPointer))
    ...
    case VTK_UNSIGNED_SHORT:
      if(bUseGPU)
      {
        this->RenderIsoSurfaceOnGPU(renderer, volume, (const unsigned short*)dataPointer);
      }
      else
      {
    ...
    case VTK_FLOAT:
      ...
      if(bUseGPU)
      {
        this->RenderIsoSurfaceOnGPU(renderer, volume, (const float*)dataPointer);
      }
      else
      {
        if (PrepareAccelerationDataTemplate((const float*)dataPointer))
  ...
  template <typename DataType> bool vtkMAFContourVolumeMapperGPU::PrepareAccelerationDataTemplate(const DataType *dataPointer)
    //------------------------------------------------------------------------------
  {
    int lod ;
  ...
  //------------------------------------------------------------------------------
  void vtkMAFContourVolumeMapperGPU::EnableClipPlanes(bool enable)
    //------------------------------------------------------------------------------
  {
    if (enable) {
  ...
  //-------------------------------------------------------------------
  void vtkMAFContourVolumeMapperGPU::Update()
    //------------------------------------------------------------------------------
  {
  ...
  //------------------------------------------------------------------------------
  bool vtkMAFContourVolumeMapperGPU::IsDataValid(bool warnings)
    //------------------------------------------------------------------------------
  {
  ...
  int vtkMAFContourVolumeMapperGPU::GetDataType()
    //------------------------------------------------------------------------------
  {
  ...
  vtkPolyData *vtkMAFContourVolumeMapperGPU::GetOutput(int level, vtkPolyData *data)
    //------------------------------------------------------------------------------
  {
...
#pragma region CPU based Marching Cube

  //------------------------------------------------------------------------------
  // Initialize OpenGL rendering, called by DrawCache() and RenderMCubes()
  void vtkMAFContourVolumeMapperGPU::InitializeRender(bool setup, vtkRenderer *renderer, vtkVolume *volume)
    //------------------------------------------------------------------------------
  {
    glGetError(); // reset error flag
    if (setup) {
  ...
  void vtkMAFContourVolumeMapperGPU::DrawCache(vtkRenderer *renderer, vtkVolume *volume, int lod)
    //------------------------------------------------------------------------------
  {
  ...
  template<typename DataType> void vtkMAFContourVolumeMapperGPU::RenderMCubes(vtkRenderer *renderer, vtkVolume *volume, const DataType *dataPointer)
    //------------------------------------------------------------------------------
  {
    int lod ;
  ...
  template<typename DataType> void vtkMAFContourVolumeMapperGPU::CreateMCubes(int LODLevel, vtkPolyData *polydata, const DataType *dataPointer)
    //------------------------------------------------------------------------------
  {
    // select LOD
  ...
  // Called by GetOutput()
  template<typename DataType> void vtkMAFContourVolumeMapperGPU::CreateMCubes_old(int level, vtkPolyData *polydata, const DataType *dataPointer) {
    //------------------------------------------------------------------------------
  ...
  void vtkMAFContourVolumeMapperGPU::CalculateLodIncrements(int lod, int *lodxy, int *lodz) const
    //------------------------------------------------------------------------------
  {
  ...
  void vtkMAFContourVolumeMapperGPU::CalculateVoxelVertIndicesOffsets(int lod, int *offset) const
    //------------------------------------------------------------------------------
  {
    int lodxy, lodz ;
    CalculateLodIncrements(lod, &lodxy, &lodz) ;
  ...
  int vtkMAFContourVolumeMapperGPU::VoxelVolume(int lod) const
    //------------------------------------------------------------------------------
  {
    int lodxy, lodz ;
    CalculateLodIncrements(lod, &lodxy, &lodz) ;
  ...
  int vtkMAFContourVolumeMapperGPU::EstimateTrianglesFromRelevantVolume(int lod)
    //------------------------------------------------------------------------------
  {
  ...
  int vtkMAFContourVolumeMapperGPU::EstimateNumberOfTriangles(int lod)
    //------------------------------------------------------------------------------
  {
  ...
  // Only use this to help decide whether to render at this LOD or not.
  float vtkMAFContourVolumeMapperGPU::EstimateTimeToDrawDC(int lod) const
    //------------------------------------------------------------------------------
  {
  ...
  // Only use this to help decide whether to render at this LOD or not.
  float vtkMAFContourVolumeMapperGPU::EstimateTimeToDrawRMC(int lod)
    //------------------------------------------------------------------------------
  {
    // Note: Rendering is performed either by DrawCache() or RenderMCubes().
  ...
  int vtkMAFContourVolumeMapperGPU::BestLODForDrawCache(vtkRenderer *renderer)
    //------------------------------------------------------------------------------
  {
    ...
    for (int lod = 0 ;  lod < NumberOfLods ;  lod++){
      ...
      bool timeOk = (this->m_Alpha == 1) || (EstimateTimeToDrawDC(lod) < renderer->GetAllocatedRenderTime()) ; 
  ...
  int vtkMAFContourVolumeMapperGPU::BestLODForRenderMCubes(vtkRenderer *renderer)
    //------------------------------------------------------------------------------
  {
    // return level zero if auto lod not enabled
    if (!this->AutoLODRender)
  ...
  int vtkMAFContourVolumeMapperGPU::BestLODForCreateMCubes()
    //------------------------------------------------------------------------------
  {
    // return level zero if auto lod not enabled
    if (!this->AutoLODCreate)
  ...
  //------------------------------------------------------------------------------
  // Prepare contours
  template<typename DataType> void vtkMAFContourVolumeMapperGPU::PrepareContoursTemplate(const int slice, const DataType *imageData) 
    //------------------------------------------------------------------------------
  {
  ...
  void vtkMAFContourVolumeMapperGPU::CalculateDepthMatrix(double *PM) const
    //------------------------------------------------------------------------------
  {
  ...
  float vtkMAFContourVolumeMapperGPU::DepthOfVertex(double *PM, float *v) const
    //------------------------------------------------------------------------------
  {
    float zhomo, whomo, z ;
  ...
  void vtkMAFContourVolumeMapperGPU::SortTriangles(int lod, bool sortall)
    //------------------------------------------------------------------------------
  {
  ...
  template<typename DataType> const DataType* vtkMAFContourVolumeMapperGPU::PointerFromIndices(const DataType* dataPointer, int x, int y, int z)
    //------------------------------------------------------------------------------
  {
  ...
  // dataPointer is the pointer to (0,0,0) in the volume
  // Useful in debugging to check correspondence between pointer and (x,y,z)
  template<typename DataType> void vtkMAFContourVolumeMapperGPU::IndicesFromPointer(const DataType* dataPointer, const DataType* p, int *x, int *y, int *z)
    //------------------------------------------------------------------------------
  {
    int rowSize = this->DataDimensions[0] ;
    int sliceSize = this->DataDimensions[0] * this->DataDimensions[1] ;
